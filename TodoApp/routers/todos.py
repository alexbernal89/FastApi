...
#Importacia del await para funciones asincronas, async concurrencia y def sin concurrencia
    try:
        user = await get_current_user(request.cookies.get('access_token'))
        if user is None:
            return redirect_to_login()
        todos = db.query(Todos).filter(Todos.owner_id == user.get('id')).all()
        #Le pasamos el request para que se pueda renderizar la plantilla, el usuario y los todos
